...
- Added `onEncryptionError` callback to `IAgoraMediaRtcRecorderEventHandler` to support encryption error notifications
- Added `setAudioVolumeIndicationParameters` method to `AgoraMediaRtcRecorder` for configuring remote user volume callbacks
- Optimized the parameter structure of the `onAudioVolumeIndication` callback in `IAgoraMediaRtcRecorderEventHandler`
- Enhanced the `VideoMixingLayout` class, fixed the `backgroundColor` property and added support for the `backgroundImage` property
