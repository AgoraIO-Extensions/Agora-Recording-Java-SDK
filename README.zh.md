...
- 新增 `IAgoraMediaRtcRecorderEventHandler` 的 `onEncryptionError` 回调，用于支持加密错误通知
- 新增 `AgoraMediaRtcRecorder` 的 `setAudioVolumeIndicationParameters` 方法，用于配置远端用户音量回调
- 优化 `IAgoraMediaRtcRecorderEventHandler` 的 `onAudioVolumeIndication` 回调参数结构
- 增强 `VideoMixingLayout` 类，修复 `backgroundColor` 属性并新增 `backgroundImage` 属性支持
